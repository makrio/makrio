...
class Services::Facebook < Service
  ...
  def post(post, url='')
    ...
    if post.public?
      post_to_facebook("https://graph.facebook.com/me/makr_app:make", create_open_graph_params(post).to_param)
    else
  ...
  def create_open_graph_params(post)
    {:frame => "#{AppConfig[:pod_url]}#{short_post_path(post)}", :access_token => self.access_token}
  end
